...
def generate_array_procedurally(number_of_flowers,number_of_patches,patchiness_index,env_size,flowers_per_patch,silent_sim) : 
  """
  Description: 
  ...
  dist_max_to_patch = env_size - (env_size-4*perception_range)*patchiness_index  # High Patchiness = low max distance to patch center (= Closer flowers in the patch)
...
def create_environment (array_info, array_number, reuse_generated_arrays, silent_sim) : 
  ...
  if (array_info['environment_type'] == "generate") : 
    ...
    if not reuse_generated_arrays or not found_similar_array : 
      ...
      array_geometry = generate_array_procedurally(array_info['number_of_flowers'],array_info['number_of_patches'],array_info['patchiness_index'],array_info['env_size'],array_info['flowers_per_patch'],silent_sim) 
