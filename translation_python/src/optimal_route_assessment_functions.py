...
def optimal_route_assessment(array,array_geometry,bee_data,initial_probability_matrix_list,output_folder,silent_sim,optimal_route_quality,number_of_bees) : 
  ...
  if len(bee_data.index) < number_of_bees : 
    ...
    # The simulations will be done without Q_Learning for the moment and dynamic_beta will be forcedly disactivated: 
    saved_use_Q_learning = copy.deepcopy(bee_data.loc[:, "use_Q_learning"])
    saved_online_reinforcement = copy.deepcopy(bee_data.loc[:, "online_reinforcement"])
    saved_dynamic_beta = copy.deepcopy(bee_data.loc[:, "dynamic_beta"])
    saved_beta_vector = copy.deepcopy(bee_data.loc[:, "beta_vector"])

    bee_data.loc[:, "use_Q_learning"] = False
    bee_data.loc[:, "online_reinforcement"] = True
    for sim in range (number_of_simulations) : 
    ...
    best_route_sim = reformatting_best_route_sim(best_route_sim)
    ...
    bee_data.loc[:, "online_reinforcement"] = saved_online_reinforcement
    bee_data.loc[:, "dynamic_beta"] = saved_dynamic_beta
