...
CompetitiveRoute = function(bout,arrayGeometry,probabilityArray,beeData,optimalRouteQuality,silentSim)
{
  ...
  while(any(!beeData$boutFinished)) 
  {
    ...
    for(ind in stillForaging) 
    {
      ...
      if(any(probabilityArray[[ind]][indPos,-omittedDestinations]!=0))
      {
        
        if(length(potentialDestinations)>1) 
        {
          beeRoute[ind,visitNumber] = sample(potentialDestinations,size=1,
                                             prob=probabilityArray[[ind]][indPos,-omittedDestinations]);
        } else {
    ...
    for(flower in flowerInCompetition) 
    {
      ...
      for(indLost in interactionLoser)
      {
        if(indFlowerOutcome[[indLost]][beeRoute[indLost,visitNumber-1],flower]==0)
        {
          ...
          {
            probabilityArray[[indLost]][beeRoute[indLost,visitNumber-1],flower] = 
              probabilityArray[[indLost]][beeRoute[indLost,visitNumber-1],flower] * beeData$abandonFactor[indLost];
            probabilityArray[[indLost]] = NormalizeMatrix(probabilityArray[[indLost]])
          }
        ...
        if(indFlowerOutcome[[ind]][beeRoute[ind,visitNumber-1],flowerVisited]==0) 
        {
          ...
          if(whoFeeds[ind]==1)
          {
            if(!beeData$routeCompare[ind]) 
            {
              ...
              if(onlineReinforcement) 
              {
                probabilityArray[[ind]][beeRoute[ind,visitNumber-1],flowerVisited] = 
                  probabilityArray[[ind]][beeRoute[ind,visitNumber-1],flowerVisited] * beeData$learningFactor[ind];
                probabilityArray[[ind]] = NormalizeMatrix(probabilityArray[[ind]])
              }
            }
          } else {
            indFlowerOutcome[[ind]][beeRoute[ind,visitNumber-1],flowerVisited] = -1;
            ...
            {
              probabilityArray[[ind]][beeRoute[ind,visitNumber-1],flowerVisited] = 
                probabilityArray[[ind]][beeRoute[ind,visitNumber-1],flowerVisited] * beeData$abandonFactor[ind];
              probabilityArray[[ind]] = NormalizeMatrix(probabilityArray[[ind]])
            }
  ...
  for(ind in 1:numberOfBees)
  {
    ...
    if(!onlineReinforcement) probabilityArray[[ind]] = ApplyLearning(route,
                                                                     probabilityArray[[ind]],
  ...
  for(learningValue in param.learningFactor)
  {
    for(abandonValue in param.abandonFactor)
    {
      ...
      for(arrayNumber in 1:numberOfArrays)
      {
        ...
        # Create a DF of informations to be retrieve in further analyses (and remember what parameters were used).
        ...
        for(sim in 1:numberOfSimulations)
        {
          ...
          beeSequences = list();
          probabilityArray = initialProbabilityMatrices;
          
          # Bout loop
          for(bout in 1:numberOfBouts)
          {
            beeData = RebootBeeData(beeData);
            currentBout = CompetitiveRoute(bout,arrayGeometry,probabilityArray,beeData,optimalRouteQuality,silentSim);
            
            # For Sensitivity Analysis, compare probabilityArray and currentBout$Learning
            for(bee in 1:numberOfBees)
            {
              previousMatrix = probabilityArray[[bee]];
